...
(function ($, window, document, undefined) {
  ...
  Plugin.prototype = {
    init: function (id) {
      ...
      $(function () {
        ...
        $(".rsn-toggle").on("click", function (e) {
          ...
          if($(this).hasClass("expand")){
            ...
            $("#content-container").css({"width": "100%", "max-width": "100%"});
            
          } else {
            $(this).removeClass( "collapse submit" ).addClass( "expand" ).text("Expand");
            $("#details-container").show();
            $("#content-container").css({"width": "calc(100% - 350px - 14px)", "max-width": "calc(100% - 350px - 14px)"});
          }
        });
        // end expand function
        ...
        // autocomplete
        var rsnAutocomplete = function(){

          $(".rsn-autocomplete input").focus(function() {
            // console.log('focused');
            var $trow = $(this).closest("tr"),
                status = $trow.find('.rsn-status'),
                statusVal = $trow.find('.rsn-status textarea'),
                school = $trow.find('.rsn-autocomplete input'),
              ...
              onSelect: function(suggestion) {
               //  console.log(suggestion);
                ...
                if (suggestion.data === null) {
                  ...
                  if(isSchool){
                    $(row.status).removeClass("active").removeClass("disabled").addClass("success");
                    $(row.statusVal).val('1');
                    $(row.postcode).val(suggestion.data.postcode);
                    $(row.schooldata).val(suggestion.data.urn);
                    $(row.role).val('').attr('selected', 'selected');
                  } else {
                    $(row.status).removeClass("active").removeClass("success").addClass("disabled");
                    $(row.statusVal).val('2');
                    $(row.schooldata).val('');
      ...
      $.initialize(".rsn-autocomplete input", function() {
        rsnAutocomplete();
        $('.rsn-autocomplete input:last').focus();
